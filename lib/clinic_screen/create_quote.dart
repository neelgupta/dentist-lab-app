...
class _CreateQuoteState extends State<CreateQuote> {
  ...
  void initState() {
    ...
    if (widget.quotesData != null) {
      ...
      if (priorityValue == 'urgent') {
        dateInputController.text = DateFormat('yyyy-MM-dd').format(
            DateTime.parse(
                widget.quotesData!.tillDate ?? DateTime.now().toString()));
      }
                                ...
                                child: ButtonTheme(
                                  ...
                                  child: DropdownButtonHideUnderline(
                                    child: DropdownButton<ServiceData>(
                                      ...
                                      items: services
                                          .map<DropdownMenuItem<ServiceData>>(
                                              (item) {
                                              ...
                                              : Stack(
                                                  ...
                                                  children: [
                                                    Container(
                                                      margin: EdgeInsets.all(
                                                          width * 0.01),
                                                      decoration: selectedImages[index]
                                                                  .type ==
                                                              "file"
                                                          ? BoxDecoration(
                                                              borderRadius:
                                ...
                                child: TextButton(
                                    onPressed: () {
                                      if (formKey.currentState!.validate()) {
                                        if (selectedService.isEmpty) {
                                        ...
                                        } else if (quoteTypeValue == "labList" &&
                                          ...
                                          Utils.showErrorToast(
                                              "Please Select Minimum 4 Images");
                                        } else {
                                          if (widget.quotesData == null) {
                                            createQuote();
                                          } else {
  ...
  getAllLabs() async {
    labNames.clear();
    var body = {"search": labsController.text.trim()};
    Response response = await clientProfile.getLabNames(body);
    if (response.statusCode == 200) {
      lab = Lab.fromJson(jsonDecode(response.body));
      for (var element in (lab!.labData ?? [])) {
        if (element.labName != null &&
            (element.labName ?? "").isNotEmpty &&
            !selectedLabs.contains(element)) {
  ...
  createQuote() async {
    ...
    for (var element in selectedLabs) {
      labIds.add(element.id);
    }
    var body = {
      ...
      if (dateInputController.text.isNotEmpty)
        "tillDate": dateInputController.text
    };
    Response response = await quoteService.createQuote(
        body: body,
  ...
  updateQuote() async {
    ...
    Response response = await quoteService.updateQuote(
        body: body,
        images: newImages,
        serviceId: serviceIds,
        labId: labIds,
