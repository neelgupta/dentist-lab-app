...
class _ServicesScreenState extends State<ServicesScreen> {
            ...
            child: isLoading?Center(child: loader()):SingleChildScrollView(
              ...
              child: Column(
                ...
                children: [
                  ...
                  getServicesList.isNotEmpty?Padding(
                                  ...
                                  children: [
                                    Row(
                                      children: [
                                        InkWell(onTap: () {
                                          deleteService(index);
                                        },child: const Image(height: 20,image: AssetImage("assets/image/Delete.png"))),
                                        SizedBox(width: width * 0.05),
                                        Column(
                                          crossAxisAlignment: CrossAxisAlignment.start,
                                          children: [
                                            Text(getServicesList[index].title,style: GoogleFonts.lato(fontSize: 18,fontWeight: FontWeight.w500,),),
                                            SizedBox(height: height * 0.005,),
                                            Text("AED ${getServicesList[index].price}",style: GoogleFonts.lato(fontSize: 14,fontWeight: FontWeight.w400,color: const Color(0xFFA0A0A0)),)
                                          ],
                                        ),
                                        const Spacer(),
                                        const Icon(Icons.navigate_next,color: Color(0XFF707070),)
                                      ],
                                    ),
                                    SizedBox(height: height*0.010,),
                                    const Divider(
                  ...
                  ):Container(height: height*0.5,alignment: Alignment.center,child: const Text("No Service Found !!!"),),
                ],
              ),
            ),
          ),
  ...
  deleteService(index) async {
    ...
    Response response = await LabServices.deleteService(getServicesList[index].id);
